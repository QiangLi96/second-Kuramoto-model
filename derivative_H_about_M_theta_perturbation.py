...
for  k in range(0,num_points):
    ...
    B_matrix[N:,:] = np.dot(linalg.inv(M_matrix),V_matrix**0.5)*0.0
    ...
    B_derivative[N:,:] = -np.dot(Phi_matrix,np.dot((linalg.inv(M_matrix))**2,V_matrix**0.5))*0.0
...
print H_derivative
np.save("Energy_expended_H_derivative_M_theta_perturbation_S_is_M.npy",H_derivative)
...
plt.plot(M_range,H_derivative,'*-',color='blue',label='theta_perturbation',alpha=0.5)
