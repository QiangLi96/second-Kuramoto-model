...
H_numerical_range = np.zeros((N,20,num_points))


for k_p in range(0,num_points):
    np.random.seed(3*k_p)
    perturbation_range = np.random.normal(mu_range[k_p],sigma,20)
    for k in range(0,20):
        delta_P0 = np.ones((N, 1)) * perturbation_range[k]
        V_matrix_2 = np.diag(delta_P0[:, 0])

        delta_P1 = np.ones((N, 1)) * perturbation_range[k]
        ...
        for j in range(0, 1):
            ...
            H_numerical_range[:,k,k_p] = (h_step/3.0)*(Sum[:,0]+4.0*np.sum(Sum[:,1:Total_steps:2],axis=1)+2.0*np.sum(Sum[:,2:Total_steps-1:2],axis=1)+Sum[:,-1])
        #print(k)
    print(k_p)

np.save("Energy_expen_H_motif_3_single_point_numerical_perturbation_range_nonlinear.npy",H_numerical_range)
