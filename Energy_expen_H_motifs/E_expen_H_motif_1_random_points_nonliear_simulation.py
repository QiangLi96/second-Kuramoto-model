...
np.random.seed(20)
theta_perturb = np.random.normal(mu,sigma,num_points)
...
H_numerical_range = np.zeros((N,num_points))
...
for k in range(0,num_points):
    ...
    for j in range(0, 1):
        ...
        H_numerical_range[:,k] = (h_step/3.0)*(Sum[:,0]+4.0*np.sum(Sum[:,1:Total_steps:2],axis=1)+2.0*np.sum(Sum[:,2:Total_steps-1:2],axis=1)+Sum[:,-1])
    print(k)

#np.save("E_expen_H_motif_1_random_points_nonlinear_simulation.npy",H_numerical_range)
